...
export function setCharTimeline(
  ...
  if (window.innerWidth > 1024) {
    // DOM animations: character-model shift, landing text, character-rim, what-box (same for 3D and 2D)
    tl1
    ...
    // 3D-only: character rotation, camera, monitor, screenLight, neckBone
    if (character) {
      tl1.fromTo(character.rotation, { y: 0 }, { y: 0.7, duration: 1 }, 0);
      if (camera) tl1.to(camera.position, { z: 22 }, 0);
      ...
      if (monitor?.material) tl2.to(monitor.material, { opacity: 1, duration: 0.8, delay: 3.2 }, 0);
      if (screenLight?.material) tl2.to(screenLight.material, { opacity: 1, duration: 0.8, delay: 4.5 }, 0);
      if (monitor?.position) {
